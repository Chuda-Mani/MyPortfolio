import darkSaasLandingPage from "@/assets/images/dark-saas-landing-page.png";
import lightSaasLandingPage from "@/assets/images/light-saas-landing-page.png";
import aiStartupLandingPage from "@/assets/images/ai-startup-landing-page.png";
import Image from 'next/image';
import CheckCircleIcon from "@/assets/icons/check-circle.svg";
import ArrowUpRightIcon from "@/assets/icons/arrow-up-right.svg";
import grainImage from "@/assets/images/grain.jpg";
import { SectionHeader } from "@/components/SectionHeader";
import { Card } from "@/components/Card";
import Project1 from "@/assets/images/pro1.png"
import Project2 from "@/assets/images/pro2.png"

const portfolioProjects = [
  {
    company: "Acme Corp",
    year: "2022",
    title: "Sleek Portfolio for Modern Web Developers",
    results: [
      { title: "Clean and structured layout for easy navigation" },
      { title: "Showcases key projects and skills with visual appeal" },
      { title: "Ready for future enhancements to improve responsiveness" },
    ],
    link: "https://chuda-mani.github.io/PortfolioPalette/",
    image: Project1,
  },
  
  {
    company: "Quantum Dynamics",
    year: "2023",
    title: "Fresh Produce Web App for Fruit & Vegetable Sales",
    results: [
      { title: "Streamlined product browsing for quick purchases" },
      { title: "Enhanced customer engagement with an intuitive interface" },
      { title: "Integrated features for boosting sales and expanding market reach" },
    ],
    link: "https://chuda-mani.github.io/Freshify/",
    image: Project2,
  },
];

export const ProjectsSection = () => {
  return (
    <section className="pb-16 lg:py-24">
      <div className="container">
        <SectionHeader title={"Featured Projects"} eyebrow={"Real-World Results"} description={"See how I transformed concepts into engaging digital experiences."}/>
        
        
        <div className="mt-10 md:mt-20 flex flex-col gap-20">
          {portfolioProjects.map((project , projectIndex) => (
            <Card key={project.title} className="px-8 pt-8 pb-0 md:pt-12 md:px-10 lg:pt-16 lg:px-20 sticky" style={{
              top :` calc(64px + ${projectIndex * 40}px`, 
            }}>

              

              <div className="lg:grid lg:grid-cols-2 lg:gap-16">
                {/* Text Section */}
                <div className="lg:pb-16">
                  <div className="bg-gradient-to-r from-emerald-300 to-sky-400 inline-flex gap-2 font-bold uppercase tracking-widest text-sm text-transparent bg-clip-text">
                    <span>{project.company}</span>
                    <span>&bull;</span>
                    <span>{project.year}</span>
                  </div>

                  <h3 className="font-serif text-2xl mt-2 md:mt-5 md:text-4xl">{project.title}</h3>
                  <hr className="border-t-2 border-white/5 mt-4 md:mt-5" />
                  
                  <ul className="flex flex-col gap-4 mt-4 md:mt-5">
                    {project.results.map((result) => (
                      <li key={result.title} className="flex gap-2 text-sm md:text-base text-white/50">
                        <CheckCircleIcon className="h-5 w-5 md:h-6 md:w-6" />
                        <span>{result.title}</span>
                      </li>
                    ))}
                  </ul>

                  <a href={project.link}>
                    <button className="bg-white text-gray-950 h-12 w-full md:w-auto px-6 rounded-xl font-semibold inline-flex items-center justify-center gap-2 mt-8">
                      <span>Visit Live Site</span>
                      <ArrowUpRightIcon className="h-4 w-4" />
                    </button>
                  </a>
                </div>

                {/* Image Section */}
                <div className="relative lg:flex lg:items-center">
                  <Image
                    src={project.image}
                    alt={project.title}
                    width={500} 
                    height={300} 
                    className="rounded-lg object-cover mt-8 -mb-4 md:-mb-0 lg:mt-0 lg:absolute lg:h-full lg:w-auto lg:max-w-none"
                  />
                </div>
              </div>
            </Card>
          ))}
        </div>
      </div>
    </section>
  );
};
